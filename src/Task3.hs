...
instance Num a => Num (Series a) where
  ...
  (Series (Stream x' xs)) + (Series (Stream y ys)) = Series $ Stream (x' + y) (coefficients (Series xs + Series ys))
  (Series (Stream x' xs)) * (Series (Stream y ys)) = Series $ Stream (x' * y) (coefficients (x' *: Series ys + Series xs * Series (Stream y ys)))  
  abs s = s
  signum _ = 1
...
instance Fractional a => Fractional (Series a) where
  ...
  (Series (Stream x' xs)) / (Series (Stream y ys)) = Series $ Stream (x' / y) (coefficients ((Series xs - (x' / y) *: Series ys) / Series (Stream y ys)))
